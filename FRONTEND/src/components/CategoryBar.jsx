...
const CategoryBar = ({ category }) => {
    ...
    useEffect(() => {
        const fetchCategory = async () => {
            try {
                const response = await API.post(`/products/category/`, { category }); // Use POST and send category in the body
                setRelatedProducts(response.data.products);
                setIsLoading(false); // Set isLoading to false when data is fetched
            } catch (error) {
                console.log("Personalized recommendations failed:", error);
    ...
    return (
        <div className="relative m-7">
            <h1 className="text-xl font-bold mb-4 mt-5">Shop By {category} Category</h1>
            <div
                ref={scrollRef}
